...
static async crearElemento(req: Request, res: Response) {
  try {
    ...
    if (!Nombre || !Imagen || !Cantidad) {
      res.status(400).json({ mensaje: 'Nombre, imagen y cantidad son obligatorios' });
      return;
    }

    if (!req.file || !mimeTypesPermitidos.includes(req.file.mimetype)) {
      res.status(400).json({ mensaje: 'Tipo de archivo no permitido. Usa JPG, PNG o WEBP.' });
      return;
    }
  ...
  static async getCatalogo(req: Request, res: Response) {
    try {
      const elementos = await Elemento.findAll({ order: [['createdAt', 'DESC']] });
    res.json(elementos);
    return;
    } catch (error) {
    ...
    try {
      ...
      const alquileresActivos = await AlquilerElementos.findAll({
        where: {
      ...
      if (alquileresActivos.length > 0) {
        ...
        const elementoReal = await Elemento.findByPk(elementoCatalogo.IdAlquiler);
        ...
        res.status(200).json({
          mensaje: "Elemento con alquileres activos. Marcado como no disponible y se notificó al administrador.",
        });
        return;
      }
